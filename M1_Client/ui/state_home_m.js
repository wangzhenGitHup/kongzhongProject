...
/*****************************************************************/
	...
	//	appEnv.runSnow();
		...
		if(!(window.aisGame && window.aisGame.bVisit)){
			...
			if(aisGame.curCity.cubeStorage)
				this.tokenBox=cssLib.boxResBar.create(this.hudTRBox,[220,140,0],aisGame.curCity.cubeStorage,aisEnv.defLib.prdct.ResCube,[55,100,30]);
			else
...
//		DBOut("======||||| "+this.city.vipBuyGold+" "+this.city.vipBuyOil+" "+this.city.resetDailyTasksTime);
		...
		{
			/**/
			if(window.aisGame.curCity.aisMacShop && window.aisGame.curCity.aisMacShop.level)
			{
				keyid=appEnv.hudKeys.getKey(this);
				...
				this.limitSell=this.hudBaseBox.appendNewChild({type:"key",pos:[sellX,sellY,0],anchor_h:1,anchor_v:1,css:sellPic,ui_event:1,key:keyid,diwn_scale:0.95,
					state_up:sellPic,state_down:sellPic,state_grage:sellPic,audio:this.page.audioObject.genFileURL("btn_click"),flash:2});
	...
	__Page.stateHome.addMonthCard15Icon = function()
	{
		var cssLib=this.page.cssLib;
		if(!this.thBox){
			var keyid=this.appEnv.hudKeys.getKey(this);
			this.regKeyVO(keyid,this,this.onAddGemClick);
			this.thBox=cssLib.boxTH.create(this.hudTLBox,[280,84,0],aisGame.curCity,keyid);
...
//			}
		...
		{
			this.appEnv.newMsg(["M1Dwr","visitPlayer",userId],{cb:function(svrVO){
				this.go2Visit(svrVO);
			},cbobj:this},window.ReplayDWRUrl);
		};
		...
		__Page.stateHome.loginClan=function()
		{
			...
			this.king.sendNTCmd(this.king,"LoginClan",{callBack:function(vo){
				...
				this.page.setCookie(cookieDomain,"ClanFlag",vo.clanVO.flag+"",0);
				this.page.setCookie(cookieDomain,"ClanCookie",toJSON({userId:window.USERID, clanFlag:vo.clanVO.flag}),0);
				//登录clan回调
				//ChatMsgVO msgs;
				//String clanName;
				//int clanFlag;
