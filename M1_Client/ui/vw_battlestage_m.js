...
//***************************************************************************
{
	...
	__Page.vwBattleStage.initCityBlds=function()
	{
		...
		for(i=0;i<n;i++)
		{
			...
			if(vo.c=="RGunTower" && vo.attackType)vo.c="Alt"+vo.c;
			...
			if(vo.u && vo.u.length)
			{
				...
				{
					idx=levels.getObjDefIdx(vo.u[j].codename+vo.u[j].level);
	...
	__Page.vwBattleStage.proxyBattle=function(svrVO)
	{
		...
		if(n)
		{
			...
			for(i=0;i<n;i++)
			{
				...
				idx=levels.getObjDefIdx(vo.codename+vo.level);
				for(j=0;j<vo.num;j++)
					units.push({def:idx,group:group,id:-1,jump:0});
				this.unitNum+=vo.num;
			}
	...
	__Page.vwBattleStage.replaceObjDef=function(def_name,body,leg,plug,bfoe)
	{
		...
		defVO={
			...
			move:{speed:leg.move.speed,fly:0,jump:0},
			combat:{
	...
	__Page.vwBattleStage.getBattleLog=function()
	{
		...
		if(this.state==2)
		{
			...
			if(this.TrainingTime)
			{
				...
				this.recoverUnitGem=Math.floor(this.recoverUnitGem*window.aisEnv.defLib.globals["INSTANT_RECOVERY_GEM_MODIFY"]);
			}
	...
	__Page.vwBattleStage.saveBattleReplay=function(endTick)
	{
		...
		list=this.page.getCookie("M1Replay","SelfBattleReplay"+window.USERID);
		if(!list){
			list=[];
		}else{
			list=fromJSON(list);
		...
		this.page.setCookie("M1Replay","SelfBattleReplay"+window.USERID,toJSON(list),0);
	};
