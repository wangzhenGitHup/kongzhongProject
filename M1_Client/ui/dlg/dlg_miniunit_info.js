if(!__Page.dlgMiniUnitInfo)
{
	__Page.dlgMiniUnitInfo=new __Page.gamePage.dlgBase(__Page,__Page.gamePage.appEnv,1);
	__Page.dlgMiniUnitInfo.loadConfig=function()
	{
	...
	__Page.dlgMiniUnitInfo.init=function(appEnv)
	{
		this.name="dlgMiniUnitInfo";
		this.viewId="dlgMiniUnitInfo";
		this.slotVsn=0;
		...
		this.regKeyVO(homeDlg.btnBackKeyId,this,this.onBackClk);
	...
	__Page.dlgMiniUnitInfo.enter=function(preState,vo)
	{
		//TODO:code this:
		...
		//根据VO初始化界面:
		this.infoVO=vo.def;
		this.homeDef=vo.homeDef;
		// DBOut(toJSON(this.infoVO));
		if(this.infoVO)
		{
			homeDlg.dlgTitle._setText(this.infoVO.name);
			this.descTxt.setText(this.infoVO["desc"]);

			this.picObj.setTexURL(imgLib.genIconPath("icon_"+this.infoVO.icon+"512_32"));
			this.initUI();
			if("UntDoctor"==this.infoVO.codeName)
			{
				keyid=appEnv.hudKeys.getKey(this);
				this.regKeyVO(keyid,this,this.onMoreInfoClk);
				this.moreInfoBtn=this.cntBox.appendNewChild({css:cssLib.btnInfo.createCSS([this.descCSS.pos[0]+this.descCSS.w/2,this.descCSS.pos[1],0],keyid)});
			}
		}
		this.appEnv.hudIn(this.cntBox,5);
	};
	__Page.dlgMiniUnitInfo.leave=function(nextState)
	{
		var i,list,n;
	...
	__Page.dlgMiniUnitInfo.initUI=function()
	{
		...
		this.infos=[
			...
			//若造兵需要消耗多种资源，则需要修改此处
			// {name:"TrainingCost", icon:"res_"+resCost[curLv["cost"]["storage"][0]["type"]], cur:curLv["cost"]["storage"][0]["num"], max:maxLv["cost"]["storage"][0]["num"]},
		];
		this.initInfoBar();
	...
	__Page.dlgMiniUnitInfo.initInfoBar=function()
	{
	...
	__Page.dlgMiniUnitInfo.initAttrText=function()
	{
	...
	//--------------------------------------------------------------------------
	{
		__Page.dlgMiniUnitInfo.onKey=function(msg,key,way,extra)
		{
			var ret;
		...
		__Page.dlgMiniUnitInfo.onCloseClk=function(msg,key,way,extra)
		{
			return this.homeDlg.onCloseClk(msg,key,way,extra);
		};
		__Page.dlgMiniUnitInfo.onBackClk=function(msg,key,way,extra)
		{
			if(msg==1 && way==1)
			{
				var def=this.homeDef;
				this.appEnv.switchDlg(this.dlgPage.dlgTrainInfo,0,def);
			}
		};
	}
}
