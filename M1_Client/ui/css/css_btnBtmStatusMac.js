this.cssLib.btnBtmStatusMac=
{
	...
	createCSS:function(bld)
	{
		...
		if(bld.craft.state)
		{
			img_css=imgLib.btn_macoff;
		}else{
			img_css=imgLib.btn_macon;
		}
		css={
			type:"key",id:"BtmBtnAmmoReload","pos":[0,0,0],key:appEnv.appKeys.bldMacMode,"css":img_css,"anchor_h":1,"anchor_v":1,ui_event:1,filter:1,button:1,down_scale:0.95,
		...
		{
			var url=page.genPageURL(window.imgPath+"/icon/icon_noATK64_32.png");
			css.items.push({type:"icon",pos:[0,0,0],tex:url,tex_w:1,tex_h:1,tex_u:0,tex_v:0,w:64,h:64,"anchor_h":1,"anchor_v":1,flash:2,});
	...
	update:function(bld)
	{
		...
		var def=bld.def;
		if(bld.craft&& bld.craft.state)
		{
			this.setStateStyle(2,{css:imgLib.btn_macoff});
		}else{
			this.setStateStyle(2,{css:imgLib.btn_macon});
		}
	},
	isOK2Show:function(bld)
	{
		...
		if(bld.constructing || !bld.craft || bld.working)
		{
			return 0;
		}
		return 1;
