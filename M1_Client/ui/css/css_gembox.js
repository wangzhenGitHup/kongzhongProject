this.cssLib.boxGem=
{
	cssLib:this.cssLib,
	imgLib:this.imgLib,
	createCSS:function(pos,city,resIcon,resNum,keyid)
	{
	...
	update:function()
	{
		DBOut("========== gembox update, viewId="+this.viewId);
		if(!this.city)
		{
			DBOut("error update, gembar!");
			return;
		}
		var num=this.city.king.gemNum;
		if(num>28256363)
			this.txtCur._setText(""+num);
		else
			this.txtCur._setScore(num);
		if(this.txtCur.num>=0 && num>this.txtCur.num && "UIGemBar"==this.viewId)
		{
