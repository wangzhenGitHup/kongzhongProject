...
//***************************************************************************
{
	...
	__Page.vwReplayStage.onUnitDead=function(list)
	{
		...
		{
			unit=list[i];
			if(unit.getGroup()==3 || unit.getGroup()==11){//敌人的兵
				...
				if(def)
				{
					self.page.stateReplay.refreshOppDeads({codename:def.codename,level:def.level,num:1});
	...
	__Page.vwReplayStage.addFoeScore=function(obj,num)
	{
		var idx;
	...
	__Page.vwReplayStage.genLevelDefVO=function()
	{
		...
		this.mechHPC=this.page.stateReplay.gameVO.mechHPC;
		//修正战斗单位的combat属性:
	...
	{
		if(!svrVO)return;
		var i,j,k,n,idx,vo,levels,list,svrCity,city,objs,cocBld,res,rate,clan,info;
		levels=this.level;
		this.battleInfo={stage:{},log:{cmds:[],hps:[],endTick:0},mechs:[],foeMechs:[]};
		...
		info=fromJSON(svrVO.cityInfo);
		this.unitScoreC=info.unitScoreC;
		this.remainTimesScoreC=info.remainTimesScoreC;
		this.treatC=info.treatC;
		this.mechHPC=info.mechHPC;
		this.mechScoreC=info.mechScoreC;
