if(!window.aisAutoMine)
{
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
//基础的产矿的对象类型
window.aisAutoMine=function(env,city,def)
{
	...
	if(env)
	{
		...
		this.harvestable=0;
		this.tgtStorage=city[def.mineStorage]?city[def.mineStorage]:null;
		...
		//矿的可由用户收集的最小已开采资源量
		this.addValue("harvestNum",0,null,null,null);
		//矿的可由用户收集的最小已开采资源量
...
window.aisAutoMine.ERR_NoHarvest={level:100,error:"No res to harvest!","class":"aisAutoMine",code:"ERR_NoHarvest"};
window.aisAutoMine.ERR_BadPower={level:100,error:"Power rate is out of range!","class":"aisAutoMine",code:"ERR_BadPower"};

//---------------------------------------------------------------------------
window.aisAutoMine.prototype=new window.aisBuilding();

//---------------------------------------------------------------------------
//用于UI的函数---------------------------------------------------------------
//---------------------------------------------------------------------------
{
	window.aisAutoMine.prototype.ui_onClick=function(uiEnv,state)
	{
		var page=uiEnv.page;
		var tech,level;
...
//---------------------------------------------------------------------------
window.aisAutoMine.prototype.readDef=function()
{
	var def=this.def;
...
//从Building继承来的函数:
{
	//---------------------------------------------------------------------------
	//从voObj中读出建筑数据
	window.aisAutoMine.prototype.readFmVO=function(king,voObj,def)
	{
	...
	//---------------------------------------------------------------------------
	//将建筑数据存入voObj
	window.aisAutoMine.prototype.saveToVO=function(king,voObj)
	{
	...
	//---------------------------------------------------------------------------
	//更新建筑数据
	window.aisAutoMine.prototype.updateByKing=function(king,nowTime,timeGap)
	{
...
//---------------------------------------------------------------------------
window.aisAutoMine.prototype.updateMine=function(king,nowTime,timeGap)
{
	...
	//DBOut("Cur mine: "+num+" vs "+this.getValue("harvestNum"));
...
//---------------------------------------------------------------------------
{
	//---------------------------------------------------------------------------
	//收取资源
	window.aisAutoMine.prototype.com_PowerCostRate=function(comVO)
	{
		var rate,pow,freePow;
		rate=comVO.rate;
		if(rate<0 || rate>100)
		{
			DBOut("aisAutoMine.com_PowerCostRage Error 1: bad rate: "+rate);
			throw aisAutoMine.ERR_BadPower;
		}
		pow=rate*this.getValue("powCostPerRate")
		freePow=this.city.power.getCurFreePower()+this.getPowerWorkCost()-pow;
		if(freePow<0)
		{
			DBOut("aisAutoMine.com_PowerCostRage Error 2: not enough city power.");
			throw aisAutoMine.ERR_BadPower;
		}
	...
	//---------------------------------------------------------------------------
	//收取资源
	window.aisAutoMine.prototype.com_Harvest=function(comVO)
	{
		var num;
		if(!this.harvestable)
		{
			//DBOut("aisAutoMine.com_Harvest Error 1: no res to harvest: "+this.getValue("mineCurNum")+" vs "+this.getValue("harvestNum"));
			throw aisAutoMine.ERR_NoHarvest;
		}
		//DBOut("Will harvest: "+comVO.harvestNum);
		num=this.harvestMine(comVO.harvestNum,comVO);
		comVO.harvestNum=num;
	};

	//---------------------------------------------------------------------------
	//建造/升级完成
	window.aisAutoMine.prototype.com_ConstructDone=function()
	{
...
//---------------------------------------------------------------------------
window.aisAutoMine.prototype.getMineNum=function()
{
	return this.getValue("mineCurNum");
};

//---------------------------------------------------------------------------
window.aisAutoMine.prototype.getMineCapRate=function()
{
	return this.getValue("mineCurNum")/this.getValue("mineMaxNum");
};

//---------------------------------------------------------------------------
window.aisAutoMine.prototype.isHarvestOverload=function()
{
	var num,vo;
	num=this.getValue("mineCurNum");
	vo={type:this.mineRes.codeName,num:num};
	if(!this.tgtStorage.checkPutIn(vo))
	{
		return {"all":num,"storage":vo.storeNum,"gap":num-vo.storeNum};
	}
	return 0;
};

//---------------------------------------------------------------------------
window.aisAutoMine.prototype.harvestMine=function(voNum,comVO)
{
	var num,vo,rate;
	num=Math.floor(this.getValue("mineCurNum"));
	if(comVO.critRate)
	{
		comVO.critNum=Math.round(num*comVO.critRate/100)
		num+=comVO.critNum;
	}
...
//---------------------------------------------------------------------------
//工作能量变化的回调
window.aisAutoMine.prototype.onPowWorkRate=function(v)
{
	var rate;
